...
## 部署流程
**除非用户明确要求，否则不主动部署到 Cloudflare Pages。** 开发完成后只做本地验证（`pnpm build` + 必要的 grep/smoke），部署是用户的决定，不是默认动作。

用户要求部署到稳定域名时，按照 `docs/skills/hiphone-cloudflare-pages-deploy/SKILL.md` 中的流程执行：`pnpm build` → `npx -y wrangler pages deploy dist --project-name mini-iphone --branch main --commit-dirty=true` → 验证 https://mini-iphone.pages.dev/
